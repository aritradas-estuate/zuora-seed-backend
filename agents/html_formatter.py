...
def _restore_code_blocks(text: str, code_blocks: List[Tuple[str, str]]) -> str:
    ...
    for i, (lang, code) in enumerate(code_blocks):
        ...
        if lang:
            html_block = f'<pre><code class="language-{lang}">{escaped_code}</code></pre>'
        else:
...
def _wrap_paragraphs(text: str) -> str:
    ...
    for line in lines:
        ...
        if (stripped.startswith('<') or
            ...
            stripped.startswith('__INLINE_CODE_')):
            result.append(line)
            continue

        # Wrap plain text in paragraphs
        if not any(stripped.startswith(tag) for tag in
                   ['<h', '<p', '<ul', '<ol', '<li', '<table', '<tr', '<td', '<th',
                    '<hr', '<pre', '<code', '<strong', '<em', '<label', '</','<thead', '<tbody']):
...
def generate_reference_documentation(payload_structure: Dict[str, Any]) -> str:
    ...
    rate_plans = (payload_structure.get("productRatePlans") or
                  payload_structure.get("ProductRatePlans") or
                  [])
    ...
    if isinstance(rate_plans, list):
        for i, plan in enumerate(rate_plans):
            ...
            charges = (plan.get("productRatePlanCharges") or
                      plan.get("ProductRatePlanCharges") or
                      [])

            if isinstance(charges, list):
                for j, charge in enumerate(charges):
                    charge_name = charge.get("Name") or charge.get("name") or f"Charge {j+1}"
